...
void main()
{
	...
	vec4 albedoColor = texture(uTexAlbedo, texOffset) + vec4(colorShift, 0.f);
	if(albedoColor.a < 0.5)	discard;
	
	outEmissive = albedoColor.rgb;
	...
	outColor = vec3(0.f);
	outNormal = vec3(0.5f, 0.5f, 1.f);
	outRoughness = vec3(0.f);
	outMetallic = vec3(0.f);
}