...
{
	std::string file = "../assets/materials/" + filename + ".material";
	...
	while (!inFile.eof())
	{
		...
		{
			addTexture(rm::getTexture(lineChar), textures.size());
		}
		// Special Textures
		else if (std::sscanf(line, "textureAlbedo: %s", lineChar) == 1)
		{
			addTexture(rm::getTexture(lineChar), pbrAlbedo);
		}
		else if (std::sscanf(line, "textureEmissive: %s", lineChar) == 1)
		{
			addTexture(rm::getTexture(lineChar), pbrEmissive);
		}
		else if (std::sscanf(line, "textureRoughness: %s", lineChar) == 1)
		{
			addTexture(rm::getTexture(lineChar), pbrRoughness);
		}
		else if (std::sscanf(line, "textureMetallic: %s", lineChar) == 1)
		{
			addTexture(rm::getTexture(lineChar), pbrMetallic);
		}
