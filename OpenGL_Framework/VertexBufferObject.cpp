...
void VertexArrayObject::createVAO(GLenum vboUsage)
{
	...
	for (size_t i = 0; i < numberOfBuffers; ++i)
	{
		...
		//glVertexAttribDivisor(attrib->attributeType, attrib->instance);
	}
...
void VertexArrayObject::dynamicDraw(int num) const
{
	if (vaoHandle)
	{
		this->bind();
		glDrawArraysInstanced(primitiveType, 0, vboData[0].numVertices, num);
		this->unbind();
	}
}
