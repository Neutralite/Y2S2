...
public:
	...
	void sendUniforms()
	{
		...
		for (auto itr = vec4Uniforms.begin(); itr != vec4Uniforms.end(); itr++)
			shader->sendUniform(itr->first, (vec4)itr->second);

		// Send mat4 uniforms
		for (auto itr = mat4Uniforms.begin(); itr != mat4Uniforms.end(); itr++)
			shader->sendUniform(itr->first, (mat4)itr->second);

		// Send int uniforms
		for (auto itr = intUniforms.begin(); itr != intUniforms.end(); itr++)
			shader->sendUniform(itr->first, (int)itr->second);

		// Send float uniforms
		for (auto itr = floatUniforms.begin(); itr != floatUniforms.end(); itr++)
			shader->sendUniform(itr->first, (float)itr->second);
	}
